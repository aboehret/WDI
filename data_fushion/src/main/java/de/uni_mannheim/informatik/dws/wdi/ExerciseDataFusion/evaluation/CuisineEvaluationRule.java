...
public class CuisineEvaluationRule extends EvaluationRule<Restaurant, Attribute> {
	...
	public boolean isEqual(Restaurant record1, Restaurant record2, Attribute schemaElement) {
		if(record1.getCuisine()== null && record2.getCuisine()==null)
			return true;
		else if(record1.getCuisine()== null ^ record2.getCuisine()==null)
			return false;
		else 
			return Arrays.asList(record1.getCuisine()).containsAll(Arrays.asList(record2.getCuisine())) || 
					Arrays.asList(record2.getCuisine()).containsAll(Arrays.asList(record1.getCuisine()));
	}
